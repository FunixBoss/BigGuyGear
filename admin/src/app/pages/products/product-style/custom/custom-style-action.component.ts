...
export class CustomStyleActionComponent implements ViewCell {
    renderValue: string;
    ...
    onEdit() {
        this.styleService.updateHandleAndRowData('edit', this.rowData);
    }
    ...
    deleteStyle() {
        this.styleService.delete(this.rowData.productStyleId).subscribe(
            data => {
                ...
                this.utilsService.updateToastState(new ToastState('Delete Style Successfully!', "success"))
            },
            error => {
                this.utilsService.updateToastState(new ToastState('Delete Style Failed!', "danger"))
                console.log(error);
            }
        )
    }
